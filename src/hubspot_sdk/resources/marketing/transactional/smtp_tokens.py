...
class SmtpTokensResource(SyncAPIResource):
    ...
    def list(
        ...
        *,
        ...
        Args:
          ...
          email_campaign_id: Identifier assigned to the campaign provided during the token creation.
    ...
    def delete(
        ...
        *,
        ...
        return self._delete(
            f"/marketing/v3/transactional/smtp-tokens/{token_id}",
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=NoneType,
    ...
    def reset_password(
        ...
        *,
        ...
        if not token_id:
            raise ValueError(f"Expected a non-empty value for `token_id` but received {token_id!r}")
        return self._post(
            f"/marketing/v3/transactional/smtp-tokens/{token_id}/password-reset",
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=SmtpAPITokenView,
